...
const ToneChips: React.FC<ToneChipsProps> = ({ selected, onSelect, isFunMode }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-3 px-4 py-3.5 rounded-2xl border transition-all duration-300 text-[11px] font-bold text-left ${
                isSelected
            ...
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                ...
                <i className={`fas ${option.icon} text-xs ${isSelected ? 'text-white' : 'text-slate-400'}`}></i>
              </div>
      ...
      <div className={`p-4 rounded-2xl border border-dashed transition-colors flex items-start space-x-3 ${
        isFunMode ? 'bg-ollim-green/5 border-ollim-green/20' : 'bg-ollim-sand/5 border-ollim-sand/20'
      }`}>
        <i className={`fas fa-info-circle mt-0.5 ${isFunMode ? 'text-ollim-green' : 'text-ollim-sand'}`}></i>
